...
public class BookingAggregate
{
        ...
        DateTimeOffset creationBooking)
    {
        ...
        if (startBooking.CompareTo(DateOnly.FromDateTime(creationBooking.DateTime)) <= 0)
            throw new DomainException("Начало бронирования должно быть после текущей даты");
        if (endBooking.CompareTo(startBooking) <= 0)
            throw new DomainException("Окончание бронирования должно быть после начала бронирования");
